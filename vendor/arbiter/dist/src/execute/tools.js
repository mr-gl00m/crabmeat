...
export async function execFileWrite(args, workspace) {
...
export async function execFileRead(args, workspace, maxBytes = DEFAULT_MAX_FILE_READ_BYTES) {
    const filename = String(args["filename"] ?? "");
    if (filename.length === 0) {
        return { ok: false, error: "filename missing" };
    }
...
export function execWebSearch(args) {

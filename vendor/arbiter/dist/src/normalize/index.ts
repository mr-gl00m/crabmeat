...
function score(s: string): number {
  ...
  for (const w of TRIGGER_WORDS) {
    if (lower.includes(w)) n += 2;
  }
  for (const t of SUSPICIOUS_TOKENS) {
    if (lower.includes(t.toLowerCase())) n += 3;
  }
  return n;
}

export function normalize(input: string): NormalizeResult {
  const tags: EncodingTag[] = [];
  let current = input;

  const charsetFolded = normalizeCharset(current);
  if (charsetFolded !== current) tags.push("homoglyph");
  current = charsetFolded;

  const candidates: Array<{ tag: EncodingTag; text: string }> = [];
  const url = tryUrlDecode(current);
  if (url !== null && url !== current) candidates.push({ tag: "url", text: url });
  const b64 = tryBase64(current);
  if (b64 !== null) candidates.push({ tag: "base64", text: b64 });
  const hex = tryHex(current);
  if (hex !== null) candidates.push({ tag: "hex", text: hex });
  const rot = applyRot13(current);
  if (rot !== current) candidates.push({ tag: "rot13", text: rot });

  const baseScore = score(current);
  let best: { tag: EncodingTag; text: string; score: number } | null = null;

...
import { z } from "zod";
import { normalize, type EncodingTag } from "../normalize/index.js";
...
const PARAM_SCHEMAS: Record<IntentAction, z.ZodTypeAny> = {
  file_write: z
    .object({
...
export function parseRequest(input: ParseInput): ParseProduct | null {
  const { normalized, decodedFrom } = normalize(input.request);

  if (isCompound(normalized)) return null;
  ...
  const parsed =
    parseFileWrite(normalized, input.workspace) ??
